...
nas::nas(const nas_init_t& args, const nas_if_t& itf) :
  ...
  m_security_setting(args.security_setting)
{
                                                ...
                                                const nas_if_t&                                       itf)
{
  ...
  // Security Setting : True の場合変更なし、Flaseの場合、ESM informationを送信する (2022/07/21 Add uematsu)
  if(security_setting){
	  ...
	  // Pack NAS ESM information Request in Downlink NAS Transport msg
	  nas_tx = srsran::make_byte_buffer();
	  if (nas_tx == nullptr) {
	    nas_logger.error("Couldn't allocate PDU in %s().", __FUNCTION__);
	    return false;
	  ...
	  nas_logger.info("Downlink NAS: Sending ESM information Request");
	  srsran::console("Downlink NAS: Sending ESM information Request\n");
